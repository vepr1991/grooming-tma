...
# --- ЛОГИКА ПОДТВЕРЖДЕНИЯ (с уведомлением клиенту) ---
    ...
    if res.data:
        try:
            ...
            if client_id:
                ...
                # Формируем строку про питомца
                pet_name = appt.get('pet_name', '')
                pet_breed = appt.get('pet_breed')
                pet_info = f"{pet_name} ({pet_breed})" if pet_breed else pet_name
...
# --- ЛОГИКА ОТМЕНЫ (с уведомлением клиенту) ---
