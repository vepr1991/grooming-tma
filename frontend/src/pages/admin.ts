...
function renderCarousel() {
    ...
    // 2. Add Button (if < 5 photos & edit mode)
    if (isEditMode && currentPhotos.length < 5) {
        const addSlide = document.createElement('div');
        addSlide.className = 'flex-shrink-0 w-full h-full snap-center bg-[#182430] flex items-center justify-center cursor-pointer hover:bg-[#1f2e3f] transition-colors border-2 border-dashed border-blue-500/30';
        addSlide.onclick = () => els.photoInput.click();
        addSlide.innerHTML = `
            <div class="flex flex-col items-center gap-2 text-[#3899fa]">
                <div class="w-12 h-12 rounded-full bg-[#3899fa]/10 flex items-center justify-center">
...
function renderIndicators(activeIndex: number) {
    ...
    for (let i = 0; i < totalSlides; i++) {
        ...
        dot.className = `h-1.5 rounded-full transition-all duration-300 ${isActive ? 'bg-[#3899fa] w-3' : 'bg-[#8eadcc]/30 w-1.5'}`;
...
function initPhoneMask() {
    if (els.phone && typeof IMask !== 'undefined') {
        IMask(els.phone, { mask: '+{7} (000) 000-00-00', lazy: false });
    }
}
initPhoneMask();
    ...
    try {
        await apiFetch('/me/profile', {
            ...
            body: JSON.stringify({
                ...
                photos: currentPhotos // Send photos
            })
        });
...
async function loadProfile() {
    try {
        ...
        if (data.profile) {
            ...
            // Load photos from array
            currentPhotos = data.profile.photos || [];
            if (currentPhotos.length === 0 && data.profile.avatar_url) {
                currentPhotos.push(data.profile.avatar_url);
            }
            renderCarousel();
